...
class Fig7Trainer(BaseTrainer):
    def __init__(self, model: WildfireModel, device: torch.device, run_name: str):
        ...
        self.wind_step_interval_scale = 1
    ...
    def my_train(self, ds):
        ...
        wind_step_interval = int(ds.attrs['wind_step_interval']) * self.wind_step_interval_scale
                    ...
                    for steps in range(iter_max_steps):
                        ...
                        if steps % wind_step_interval == 0:
                            self.model.wind_towards_direction = torch.tensor(
                                ds['wind_towards_direction'][iterations], device=self.device)
                            self.model.wind_velocity = torch.tensor(ds['wind_velocity'][iterations],
                                                                    device=self.device)
    ...
    def my_evaluate(self, ds):
        ...
        with tqdm() as progress_bar:
            ...
            with h5py.File(f'{self.run_name}.hdf5', 'w') as f_out:
                ...
                f_out.attrs['manhattan'] = manhattan_distance_value

                f_out.create_dataset('affected_cell_count_outputs', data=affected_cell_count_outputs,
                                     compression='gzip')
                f_out.create_dataset('affected_cell_count_targets', data=affected_cell_count_targets,
                                     compression='gzip')
                f_out.create_dataset('output', data=np.stack(output_list, axis=0),
                                     compression='gzip')
                f_out.create_dataset('target', data=np.stack(targets_list, axis=0), compression='gzip')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--wind_step_interval_scale', type=int, required=False, default=1)

    args = parser.parse_args()

    with h5py.File('real_targets.hdf5', 'r') as f_in:
        ...
        trainer.lr = args.lr
        trainer.wind_step_interval_scale = args.wind_step_interval_scale
